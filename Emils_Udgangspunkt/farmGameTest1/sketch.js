...
function preload(){
  ...
  // Load player animation frames for moving left
  for (let i = 1; i <= 4; i++) { // Assuming 4 frames for the left animation
    skinFramesLeft.push(loadImage(`images/PlayerFrames/FarmManLeft${i}_256x256.png`)); //The left side should be wiotout the ølstykke, cause the tatoo is on the left arm so you shouldt could see it
  }
    // Load the idle frame
...
function setup() {
  ...
  // Test button to add coins and potatoes locally
  testButton = createButton("Add a coin and a potato - test button").position(width- 200, 200).mousePressed(() => {
    // Update local state only
    gameController.addCoins(1);
    gameController.addPotatoes(1);
    console.log("Local state updated: Coins = " + gameController.coins + ", Potatoes = " + gameController.potato);
  });
  testButton.style("background-color", "#0000FF");
  testButton.style("color", "white");
  testButton.style("padding", "10px 20px");
  testButton.style("border", "none");
  testButton.style("border-radius", "5px");
